...
namespace GUI
{
    ...
    {
        public delegate void Mydel(int id,string Name, string sdt, int point);
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.SelectedRows.Count == 1)
            {
                if (dataGridView1.CurrentRow.Cells[0].Value.ToString() != "")
                {
                    ...
                    d(Convert.ToInt32(row[0].ToString()),row[1].ToString(), row[4].ToString(), Convert.ToInt32(row[5].ToString()));
                    this.Hide();
                }
            }
        }
